...
export const usersApi = {
  ...
  async getInterviewers() {
    const { data, error } = await supabase.from('users').select('*').eq('role', 'Interviewer');
    if (error) throw error;
    return data;
  },
...
export const applicationsApi = {
  ...
  async updateStatus(id: string, status: string) {
    const { data, error } = await supabase.from('applications').update({ application_status: status }).eq('application_id', id).select().single();
    if (error) throw error;
    return data;
  },
...
export const roleSkillsApi = {
  async getByRole(roleId: string) {
    const { data, error } = await supabase.from('role_skills').select('*, skills(*)').eq('role_id', roleId);
    if (error) throw error;
    return data;
  },
  
  async addSkillToRole(roleId: string, skillId: string) {
    const { data, error } = await supabase.from('role_skills').insert({ role_id: roleId, skill_id: skillId }).select().single();
    if (error) throw error;
    return data;
  },
  
  async removeSkillFromRole(roleId: string, skillId: string) {
    const { error } = await supabase.from('role_skills').delete().eq('role_id', roleId).eq('skill_id', skillId);
    if (error) throw error;
  }
};
...
export const interviewerSkillsApi = {
  async getByInterviewer(userId: string) {
    const { data, error } = await supabase.from('interviewer_skills').select('*, skills(*)').eq('user_id', userId);
    if (error) throw error;
    return data;
  },
  
  async addSkillToInterviewer(userId: string, skillId: string) {
    const { data, error } = await supabase.from('interviewer_skills').insert({ user_id: userId, skill_id: skillId }).select().single();
    if (error) throw error;
    return data;
  },
  
  async removeSkillFromInterviewer(userId: string, skillId: string) {
    const { error } = await supabase.from('interviewer_skills').delete().eq('user_id', userId).eq('skill_id', skillId);
    if (error) throw error;
  },
  
  async findInterviewersBySkill(skillId: string) {
    const { data, error } = await supabase.from('interviewer_skills').select('*, users(*)').eq('skill_id', skillId);
    if (error) throw error;
    return data;
  }
};
...
export async function findInterviewersForRole(roleId: string) {
  ...
  const { data, error } = await supabase.from('interviewer_skills').select('user_id').in('skill_id', skillIds);
  ...
  const { data: interviewers, error: interviewersError } = await supabase.from('users').select('*').in('user_id', interviewerIds).eq('role', 'Interviewer');
