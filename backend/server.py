...
def supabase_request(endpoint, method='GET', data=None, headers=None, files=None):
    ...
    try:
        ...
        app.logger.error(f"Supabase request failed: {str(e)}")
        return {
...
# API endpoint for creating demo requests
...
def create_demo_request():
    try:
        ...
        if status_code >= 400:
            return jsonify({
                'success': False,
                'error': f"Failed to save demo request: {response.get('error', 'Unknown error')}"
            }), status_code
        ...
        app.logger.error(f"Error in create_demo_request: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
