...
export const getInterviews = async (): Promise<{ success: boolean; data?: any; error?: string }> => {
  try {
    const response = await fetch('/api/interviews');
    if (!response.ok) {
...
export const getInterviewById = async (id: string): Promise<{ success: boolean; data?: any; error?: string }> => {
  try {
...
export const updateInterview = async (id: string, updateData: Partial<InterviewFormData>): Promise<{ success: boolean; data?: any; error?: string }> => {
  try {
    const response = await fetch(`/api/interviews/${id}`, {
      method: 'PUT',
      headers: {
